...
- **Plans layout (nested post-cutover)** — After the scope **Phase C cutover milestone** is recorded (Master Plan §7 Caveats + **`.sedea/operations/<scope>/plans-nested-cutover.yaml`** **`activeFrom: "YYYY-MM-DD"`**), new **`plan and deliver`** dispatches on that scope use automatic nested layout at Squad Leader intake: plans land under **`.sedea/operations/<scope>/plans/YYYY-MM/<unique-dispatch-slug>/`** via **`plansBasePath`** handover on every plan-writing child spawn — **no intake AskQuestion opt-in**. **`YYYY-MM`** is the developer local timezone month at first plan write; **`<unique-dispatch-slug>`** is kebab-case dispatch title + 4-char hex suffix. **Pre-cutover** and in-flight dispatches keep flat-root **`plans/`** behavior when **`plansBasePath`** is omitted or cutover is inactive. **Forbidden:** inferring dispatch scope from folder mtime or “newest” nested directory — bind to **`dispatchId`**, explicit **`targetPlanPath`**, and spawn handover per rule **31**. Resolution detail: rule **30** § *Plans write path resolution*; PRD §5.1–§5.3.
...
**Manual tests in Before + After replica (binding):** Every **manual** §7 check must appear as a numbered task in **`### Before deploy`**. **`### After deploy`** must re-cover those same manual checks (numbered retest rows **or** one umbrella item) plus After-only production items — **`deploy-walk/SKILL.md`** § *Before/After deploy manual parity (binding)* and **`coding-session/SKILL.md`** § *Before/After deploy parity (binding)*. **Forbidden:** manuals that exist only under After deploy.
